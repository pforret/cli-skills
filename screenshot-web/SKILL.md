...
```bash
screenshot-web/.venv/bin/shot-scraper <url> [options]
```
...
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o example.png
```

Mobile viewport:
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o mobile.png -w 390 -h 844
```

Capture a specific element:
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o hero.png -s "#hero"
```

Viewport only (no scroll):
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o fold.png -w 1280 -h 800
```

Retina/HiDPI:
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o retina.png --retina
```

Execute JS before capture (e.g. dismiss a cookie banner):
```bash
screenshot-web/.venv/bin/shot-scraper https://example.com -o clean.png -j "document.querySelector('.cookie-banner')?.remove()"
```
...
1. Ensure setup has been run (`bash screenshot-web/setup.sh`)
